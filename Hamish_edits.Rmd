...
### Section 2.1 - Exploration of the Variables

Now that we have cleaned the data set, we can begin exploring it. We will start this process by looking individually at the different variables - determining the main components that make up each variable in the cleaned data set, as well as what proportion they represent. This allows us to better understand the characteristics of our sample, and draw some initial conclusions about why the data is distributed the way that it is. 
...
We now want to examine the dependent variable of this investigation, which is average annual salary (converted into USD). In order to analyse this variable, we need to group the salaries into categories...
