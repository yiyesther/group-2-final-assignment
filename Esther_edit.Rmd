...
# add percentage mark and change labels
  
  #geom_text(aes(x=1.05*sin(middle), y=1.05*cos(middle),label = Value,
                #hjust = hjust, vjust = vjust))

```

As we can see here, the majority of people surveyed fall into the age bracket of 25-44. Given that 'managers' were the target audience of this survey, we would not expect to see many respondents below this age group, and this is confirmed by the chart above. It is however, somewhat surprising that we don't see as many senior executives (between the ages of 45-64) complete the survey - this is something that we may want to factor into our analysis.  
...
As we can see, there is a fairly balanced distribution of industries, with the most common being Computing/Tech, education and (surprisingly) non-profit. The balanced spread of data between industries should allow for a fairly unbiased comparison of the salaries by industry later in this project. 
...
ggplot(df7, aes(x="", y=Value, fill=Group))+ 
  ...
  coord_polar(theta = "y") + 
  theme_void() + 
  labs(title="Percentage of people in each category of \nSECTOR-SPECIFIC experience level")
```
...
It can be seen here that Hispanic people have the highest mean earning while White people have the lowest. However, the sample size of White is a lot larger than the other ones, which makes its statistics more predictive. 
...
```
* From the above analysis, it can be seen that the largest pay gap exists between White and Asian. To further visualise the difference, a confidence interval plot is created.
...
```
It can be seen in the box plot that as the level of education increases, people tend to make more (except for "Some College". The closest boxes are Master's degree and College degree, and to confirm they are significantly different, a bootstrap analysis is used.
...
observed_difference_edu <- earning_edu$mean[3]-earning_edu$mean[1]
...
earning_exp
```
Unlike education level, working for a longer time does not guarantee a higher earning. The highest mean observed is for the group of 21 - 30 years of work experience, while people who worked for 41 years or more only ranked the 5th.
...
* As the years of experience increases, the increase in mean earnings is not necessarily significant. In other words, people's earning does not always increase significantly when they work longer. The only two significant differences in earnings are observed from 2 - 4 years to 5 - 7 years and from 11 - 20 years to 21 - 30 years.
...
```{r}
null_model <- glm(log_earnings ~ 1 , data = regression_data_dummies)
full_model <- glm(log_earnings ~ how_old_are_you  + years_of_experience_in_field  + gender_Man + gender_Woman + highest_level_of_education_completed_College_degree + highest_level_of_education_completed_High_School + highest_level_of_education_completed_Masters_degree  + race_Asian + race_White + race_Hispanic + race_Black , data = regression_data_dummies)
step(null_model, direction = "both", scope = list("lower" = null_model, "upper" = full_model))
```


```{r}
model_5 <- lm(log_earnings ~ years_of_experience_in_field + gender_Man + race_Asian + 
    how_old_are_you + highest_level_of_education_completed_High_School + 
    highest_level_of_education_completed_College_degree + highest_level_of_education_completed_Masters_degree + 
    gender_Woman + race_White + race_Hispanic + race_Black , data = regression_data_dummies)
